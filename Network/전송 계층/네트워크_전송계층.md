...
ㄴ Hop by Hop basis란 패킷이 매 라우터 또는 노드를 건너가는 모습을 표현한다고 한다.<br/> 그렇기에 데이터 패킷이 최종 목적지를 도달하기까지 지속적으로 노드를 건너가는 과정을 거친다.<br/>데이터 전송만 할 뿐, 끝과 끝을 연결하는 역할을 하지 않는다.<br/>(데이터가 완전히 전달되었는지 확인을 하지 않는다는 점이다!)
...
- 네트워크 상에서 서로 가진 데이터가 올바르게 전송되고 받을 수 있도록 data packet으로 잘게 쪼게며 IP를 통해 정확한 위치로 data packet들을 전달s하고, 보내진 Packet들은 다시 온전한 구조로 재구성된다.
